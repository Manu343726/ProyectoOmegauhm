...
public class HomeController {
	...
	public void addThreadsToSession(Model model) {
		List<Topic> threads = entityManager
				.createQuery("select t from Topic t").getResultList();
	...
	public String empty(Locale locale, Model model) {
		...
		// Mocking
		if (entityManager.createQuery("select t from Topic t").getResultList()
				.size() <= 0) {
			IWEntityManager manager = new IWEntityManager(entityManager);
			...
			for (int i = 0; i < threads; ++i) {
				...
				for (int j = 0; j < answers_per_thread; ++j)
					manager.newPost(topic, "Texto respuesta " + i + "." + j,
							"admin");
			}
		}
	...
	public String vote(@PathVariable("id") long id,
		...
		if (!this.isLogged(session))
			return "redirect:401"; // go fuck yourself
